...
const Performance = () => {
  ...
  useEffect(() => {
    const fetchPerformanceData = async () => {
      ...
      if (result.success) {
        setPerformanceData(Array.isArray(result.data) ? result.data : []);
      } else {
  ...
  const calculateStats = () => {
    if (!Array.isArray(performanceData) || performanceData.length === 0) {
      return { completed: 0, totalCourses: 0, avgScore: 0 };
    }
    const completed = performanceData.filter(data => data.marks > 0).length;
    const totalCourses = performanceData.length;
    const avgScore = performanceData.reduce((sum, data) => sum + (data.marks || 0), 0) / performanceData.length;
              ...
              <table className="min-w-full divide-y divide-gray-200">
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {performanceData.map((data, index) => {
                    const scoreData = getScoreIcon(data.marks || 0);
                    // ✅ fixed — backend returns course_name directly not data.course.course_name
