...
function Courses() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (coursesRes.success) setCourses(Array.isArray(coursesRes.data) ? coursesRes.data : []);
  ...
  const enrollCourse = async (courseId) => {
    ...
    // ✅ FIX: just check res.success — don't compare res.data string
    if (res.success) {
      message.success("Course enrolled successfully!");
      setEnrolled(prev => [...prev, courseId]); // ✅ update UI instantly
      setTimeout(() => navigate(`/course/${courseId}`), 1500);
    } else {
      message.error(res.error || "Enrollment failed. Please try again.");
    }
  };
          ...
          <>
                ...
                >
                  ...
                  <div className="p-6">
                    <h3 className="text-xl font-bold text-gray-900 mb-3 line-clamp-2 group-hover:text-blue-600 transition-colors duration-200">
                      {course.course_name && course.course_name.length < 8
                    ...
                    {enrolled.includes(course.course_id) ? (
                      <button
                        ...
                        className="w-full py-3 rounded-xl bg-gradient-to-r from-green-50 to-emerald-50 text-green-700 font-semibold hover:from-green-100 hover:to-emerald-100 transition-all duration-200 border border-green-200"
                      >
                        ✓ Enrolled
                      </button>
                    ) : (
                      <button
                        ...
                        className="w-full py-3 rounded-xl bg-gradient-to-r from-blue-600 to-blue-700 text-white font-semibold hover:from-blue-700 hover:to-blue-800 transition-all duration-200 shadow-lg hover:shadow-xl"
                      >
                        Enroll Now
                      </button>
                    )}
            ...
            {displayedCourses.length < filteredAndSortedCourses.length && (
              <div className="text-center mt-8">
                <button
                  onClick={loadMore}
                  className="px-8 py-3 bg-white border border-gray-200 text-gray-700 font-medium rounded-lg hover:bg-gray-50 transition-colors duration-200"
                >
                  Load More Courses
                </button>
              </div>
